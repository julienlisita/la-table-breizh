...
export default function Header() {
  ...
  return (
    <header className="w-full h-14 sm:h-16 lg:h-20 flex items-center justify-between px-4 bg-creme fixed top-0 z-50">
      ...
      <div className={[
        "absolute top-full left-0 w-full bg-creme text-terre flex flex-col items-center space-y-4 overflow-hidden transition-all duration-300 lg:hidden",
        isOpen ? "max-h-96 opacity-100 translate-y-0 py-4" : "max-h-0 opacity-0 -translate-y-2 py-0"
      ]
        .filter(Boolean)
        .join(" ")}>
        <Link href="/" onClick={toggleState} className="hover:bg-ocre hover:text-creme w-full px-4 py-2 rounded-md text-center transition-all duration-300">Accueil</Link>
        <Link href="/menu" onClick={toggleState} className="hover:bg-ocre hover:text-creme w-full px-4 py-2 rounded-md text-center transition-all duration-300">Menu</Link>
        <Link href="/contact" onClick={toggleState} className="hover:bg-ocre hover:text-creme w-full px-4 py-2 rounded-md text-center transition-all duration-300">Contact</Link>
      </div>
    </header>
  );
}